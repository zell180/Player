...
public class DownloadNotification implements IDownloaderClient {
    ...
    private Notification.Builder mCurrentNotification;
    ...
    public void onDownloadStateChanged(int newState) {
        ...
        if (newState != mState) {
            ...
            Notification mPopUp = mNotification.build();
            if (ongoingEvent) {
                mPopUp.flags |= Notification.FLAG_ONGOING_EVENT;
            } else {
                mPopUp.flags &= ~Notification.FLAG_ONGOING_EVENT;
                mPopUp.flags |= Notification.FLAG_AUTO_CANCEL;
            }
            mNotificationManager.notify(NOTIFICATION_ID, mPopUp);
        }
    }
    ...
    public void onDownloadProgress(DownloadProgressInfo progress) {
        ...
        if (progress.mOverallTotal <= 0) {
            ...
            Notification mPopUp = mCurrentNotification.build();
            mNotificationManager.notify(NOTIFICATION_ID, mPopUp);
        } else {
            ...
            mNotificationManager.notify(NOTIFICATION_ID, mCustomNotification.updateNotification(mContext));
        }
